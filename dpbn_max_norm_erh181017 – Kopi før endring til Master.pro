...
lokasjon='C:\Users\Eirik\Dropbox\Universitet\Master\pats\Patient 7'  
patient = (strsplit(lokasjon, '\', /EXTRACT))[-1]
...
pet_matrix_orig = pet_matrix
dp_matr_orig = dp_matr
...
;prank trenger prosent i tall fra 0 til 100, ikke 0 til 1.
if p_low gt 0.0 then min_pet=prank(pet_matrix(gtv_pet_ind), p_low)   ;sort_gtv(round(n_tot*p_low)-1)
...
p_high = 0.01*p_high
p_low = 0.01*p_low ;Endres her fordi nå må jeg gå fra 0 til 100 for prank til 0 til 1 intervall
...
d_high_l=(d_mean+d_low*(dp_skal*n_mean-n_mean-n_low))/(dp_skal*n_mean+n_high)   ;d_high found for a set d_mean
...
;     CONVOLVER FORMULA
;
;b = convolver(pet_matrix_orig, gtv_pet_ind, d_high, d_low, dp_matr, p_low*100, p_high*100, set_base, dp_matr_orig, res_x_p, res_y_p, z_skal_p)
...
;openw, 4, 'C:\Users\Eirik\Dropbox\Universitet\Master\presc_matrix.txt'
;printf, 4, dp_matr
;close, 4
;
;openw, 5, 'C:\Users\Eirik\Dropbox\Universitet\Master\gtv_pet_ind.txt'
;printf, 5, gtv_pet_ind
;close, 5


;--------------Sammenlign med Researchs doseprofil-----------
ans = ''
READ, ans, prompt = 'Har du en dosefil å sammenligne mot? (Y/n) '
if ans eq 'Y' then begin
;  dp_matr_R = read_dat("C:\Users\Eirik\Dropbox\Universitet\Master\fraction_dose.dat")
  ;a = plot_widget_with_slider(dp_matr, "C:\Users\Eirik\Dropbox\Universitet\Master\fraction_dose.dat")
  ;a = dose_difference(dp_matr, "C:\Users\Eirik\Dropbox\Universitet\Master\fraction_dose.dat", gtv_pet_ind, d_low, d_high)
  ;a = dat_treater(dp_matr, gtv_pet_ind, dp_matr_inv, pet_matrix)
  a = write_gtv_pet_ind((dat_lokasjon + patient + '_gtv_pet_ind.dat'), gtv_pet_ind)
  b = write_dat((dat_lokasjon + patient + '_dp_matr.dat'), dp_matr, n_elements(dp_matr), [xdim, ydim, zdim], [res_x, res_y, z_skal], posi, 34)
  c = write_dat((dat_lokasjon + patient + '_pet_matrix.dat'), pet_matrix, n_elements(pet_matrix), [xdim, ydim, zdim + 1], [res_x, res_y, z_skal], posi, 34)
  ;d = read_gtv_pet_ind((dat_lokasjon + patient + '_gtv_pet_ind.dat'))
  ;e = read_dat((dat_lokasjon + patient + '_dp_matr.dat'))
  ;f = read_dat((dat_lokasjon + patient + '_pet_matrix.dat'))
endif
...
if ans eq 'Y' then begin
  ...
  ;window, 1
  cd, figur_lokasjon
  hist1 = plot(l, histo, /stairstep, title = 'Preskribert dose til tumor', xtitle = 'Dose [Gy]', ytitle = '# voxler')
  hist2 = plot(l2, histo_inv, /stairstep, title = 'Invers av prekskribert dose til tumor', xtitle = 'Dose [Gy]', ytitle = '# voxler')
  hist1.Save, "presc_hist.png", BORDER=10, $
    RESOLUTION=300, /TRANSPARENT
  hist2.Save, "inv_hist.png", BORDER=10, $
    RESOLUTION=300, /TRANSPARENT
  window, 1
