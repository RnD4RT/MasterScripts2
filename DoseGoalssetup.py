...
for i in range(0, len(exsisting_list)):
	with CompositeAction('Edit Optimization Function'):
		retval_list[i] = plan.PlanOptimizations[1].EditOptimizationFunction(FunctionType="MaxEud", DoseBasedRoiFunction=plan.PlanOptimizations[1].Objective.ConstituentFunctions[0], RoiName="Parotid_L", IsConstraint=False, RestrictAllBeamsIndividually=False, RestrictToBeam=None, IsRobust=False, RestrictToBeamSet="rob_ph_dpbn", UseRbeDose=False)
	
for i in exsisting_list:
	if i == "Parotid_L":
		with CompositeAction('Edit Optimization Function'):
